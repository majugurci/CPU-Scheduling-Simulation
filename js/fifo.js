...
function pokreni_iduci () {
	if (svi_gotovi == false) {
		...
		for (i99=1; i99<=podaci_o_procesima.ukupan_broj_procesa; i99++) {
			fifo_ukupno[i99] = window["vrijeme_" + i99 + "_ukupno"]/1000;
			fifo_cekanje[i99] = window["vrijeme_" + i99 + "_animacije"]/1000;
			pomocni += fifo_cekanje[i99];
		}
		pomocni /= podaci_o_procesima.ukupan_broj_procesa;
		// na 0 indexu nalazi se prosjecno vrijme cekanja
		fifo_cekanje[0] = pomocni;
		sessionStorage.setItem("fifo_ukupno", JSON.stringify(fifo_ukupno));
		sessionStorage.setItem("fifo_cekanje", JSON.stringify(fifo_cekanje));
		sessionStorage.setItem("fifo_brzina", a);
	}
}
